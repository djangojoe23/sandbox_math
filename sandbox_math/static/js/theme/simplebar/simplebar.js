...
(function (global, factory) {
  ...
  var classof = toStringTagSupport
    ...
    : function (it) {
        ...
        return it === undefined
          ...
          ? 'Null'
          : // @@toStringTag case
          typeof (tag = tryGet((O = Object$3(it)), TO_STRING_TAG$1)) == 'string'
          ? tag
          : // builtinTag case
          CORRECT_ARGUMENTS
          ? classofRaw(O)
  ...
  var objectKeysInternal = function (object, names) {
    ...
    var key;
    for (key in O)
      !hasOwnProperty_1(hiddenKeys, key) &&
      ...
      ? function assign(target, source) {
          ...
          while (argumentsLength > index) {
            ...
            var keys = getOwnPropertySymbols
              ? concat$1(objectKeys(S), getOwnPropertySymbols(S))
              : objectKeys(S);
  ...
  var iterate = function (iterable, unboundFunction, options) {
    ...
    if (IS_ITERATOR) {
      ...
      if (!iterFn)
        throw TypeError$a(tryToString(iterable) + ' is not iterable');
    ...
    getConstructor: function (wrapper, CONSTRUCTOR_NAME, IS_MAP, ADDER) {
      ...
      defineBuiltIns(Prototype, {
        ...
        delete: function (key) {
          ...
          return (
            data && hasOwnProperty_1(data, state.id) && delete data[state.id]
          );
        },
  ...
  var handlePrototype$1 = function (CollectionPrototype, COLLECTION_NAME) {
    if (CollectionPrototype) {
      // some Chrome versions have non-configurable methods on DOMTokenList
      if (CollectionPrototype[ITERATOR$6] !== ArrayValues)
        try {
          createNonEnumerableProperty(
            CollectionPrototype,
            ITERATOR$6,
  ...
  // https://tc39.es/ecma262/#sec-array.prototype.reduce
  _export(
    { target: 'Array', proto: true, forced: !STRICT_METHOD$1 || CHROME_BUG },
    {
      ...
      return [
        ...
        function (string, replaceValue) {
          ...
          for (var i = 0; i < results.length; i++) {
            ...
            var position = max$2(
              min$2(toIntegerOrInfinity(result.index), S.length),
              0,
            );
            var captures = [];
